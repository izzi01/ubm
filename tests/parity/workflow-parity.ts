...
export interface WorkflowParityArtifactCheck {
  id: string
  producerPhase: string
  path: string
  exists: boolean
...
export interface WorkflowParityTransitionCheck {
  phase: string
  entity: string
  field: string
  expected: string
...
export async function createWorkflowParityReport(): Promise<WorkflowParityReport> {
  ...
  try {
    ...
    await inProjectDir(base, async () => {
      ...
      if (milestoneResult.details.error) {
        throw new Error(String(milestoneResult.details.error))
      }
      ...
      if (sliceResult.details.error) {
        throw new Error(String(sliceResult.details.error))
      }
      ...
      if (taskResult.details.error) {
        throw new Error(String(taskResult.details.error))
      }
    ...
    if (!finalManifest) {
      throw new Error("workflow parity fixture did not produce a state manifest")
    }
    ...
    const artifactChecks = manifest.contract.requiredArtifacts.map((artifact) => {
      ...
      const missingMarkers = exists ? artifact.mustContain.filter((marker) => !content.includes(resolveTemplate(marker, ids))) : [...artifact.mustContain]
      return {
        id: artifact.id,
        ...
        exists,
        missingMarkers,
      } satisfies WorkflowParityArtifactCheck
    })
    ...
    const taskAfterPlanSlice = snapshots["plan-slice"]?.tasks.find(
      (row) => row.id === ids.taskId && row.slice_id === ids.sliceId && row.milestone_id === ids.milestoneId,
    )
    const taskAfterComplete = finalManifest.tasks.find(
      (row) => row.id === ids.taskId && row.slice_id === ids.sliceId && row.milestone_id === ids.milestoneId,
    )
    ...
    const stateTransitions: WorkflowParityTransitionCheck[] = [
      ...
      },
      {
        ...
        status: taskAfterComplete?.verification_result === "passed" ? "passed" : "failed",
      },
    ]
    ...
    for (const artifact of artifactChecks) {
      ...
      if (artifact.missingMarkers.length > 0) {
        failureDiagnostics.push(
          `malformed artifact: ${artifact.path} missing markers ${artifact.missingMarkers.join(", ")} (phase ${artifact.producerPhase})`,
        )
      }
    }
    ...
    for (const transition of stateTransitions) {
      if (transition.status !== "passed") {
        failureDiagnostics.push(
          `invalid transition: ${transition.phase} expected ${transition.entity}.${transition.field}=${transition.expected} but saw ${transition.observed ?? "missing"}`,
        )
      }
    }
