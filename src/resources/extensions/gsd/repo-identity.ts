...
function ensureGsdSymlinkCore(projectPath: string): string {
  ...
  try {
    ...
    if (stat.isDirectory()) {
      ...
      // In worktrees, keep the directory in place and let the external state
      // symlink provide the shared state. Replacing a git-tracked .gsd directory
      // with a symlink makes git think tracked planning files were deleted.
      return localGsd;
    }
  } catch {
